...
/**
 * Report bestsuppliers collection
 *
 * @category    Mage
 * @package     Mage_Sales
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Edge_Reports_Model_Resource_Report_Topsuppliers_Collection extends Mage_Sales_Model_Resource_Report_Collection_Abstract
{
    /**
     * Rating limit
     *
    ...
    public function __construct()
    {
        ...
        $this->_resource = Mage::getResourceModel('edge_report/report')->init('edge_report/topsuppliers_aggregated_daily');
        ...
        if (!$this->_selectedColumns) {
            if ($this->isTotals()) {
                ...
                $this->_selectedColumns = array(
                    ...
                    'social_designation' => 'MAX(social_designation)',
     ...
     * Add selected data
     *
     * @return Mage_Sales_Model_Resource_Report_Bestsuppliers_Collection
     */
    protected function _initSelect()
    {
        ...
        if (!$this->_period) {
            ...
            if ($this->_from || $this->_to) {
                $mainTable = $this->getTable('edge_reports/topsuppliers_aggregated_daily');
                $select->from($mainTable, $cols);
            } else {
                $mainTable = $this->getTable('edge_reports/topsuppliers_aggregated_yearly');
                $select->from($mainTable, $cols);
            }
        ...
        if ('year' == $this->_period) {
            ...
            $mainTable = $this->getTable('edge_reports/topsuppliers_aggregated_monthly');
            $select->from($mainTable, $this->_getSelectedColumns());
        } else {
            $mainTable = $this->getTable('edge_reports/topsuppliers_aggregated_daily');
            $select->from($mainTable, $this->_getSelectedColumns());
        ...
        if (!$this->isTotals()) {
            $select->group(array('period', 'supplier_id'));
        }
